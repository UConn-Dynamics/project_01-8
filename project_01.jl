...
# ╔═╡ 30b2f4ea-4e5c-4fe6-aa73-d3f3c8be5087
md"""
## Derivations
Lets consider the inertial reference frame $x'-z'$ rotating plane which is rotating at a constant velocity $\Omega \hat{z}$. The position of the mass is in this reference frame is $\mathbf{r}' = \left(\omega_1 +  L \sin(\theta) \right) \hat{x}' + \left( h_1 - L \cos(\theta) \right) \hat{z}'$.

#### Kinetic Energy $T$
The kinetic energy can be derived from $T = \frac{1}{2}m \mathbf{v}^2$ where the velocity of the mass is $\mathbf{v} = \mathbf{v}' + \mathbf{v}_{rot}$ which includes velocity of the pendulum within the $x'-z'$ reference frame and the velocity of the rotating frame.

```math
\mathbf{v}' = \frac{d \mathbf{r}'}{dt} = L \dot{\theta} \cos(\theta)\hat{x}' + L \dot{\theta} \sin(\theta)\hat{z}'
```

```math
\mathbf{v}_{rot} = \Omega \hat{z} \times \mathbf{r}' 
= \Omega \hat{z} \times \left[ \left(\omega_1 +  L \sin(\theta) \right) \hat{x}' + \left( h_1 - L \cos(\theta) \right) \hat{z} \right] 
= \Omega \left[ \left(\omega_1 +  L \sin(\theta) \right) (\hat{z} \times \hat{x}') + \left( h_1 - L \cos(\theta) \right) (\hat{z} \times \hat{z}) \right] 
= \Omega \left[ \left(\omega_1 +  L \sin(\theta) \right) (\hat{y}') + \left( h_1 - L \cos(\theta) \right) (0) \right]
= \Omega \left(\omega_1 +  L \sin(\theta) \right) \hat{y}'
```

Therefore, $\mathbf{v}$ can be used to find $T = \frac{1}{2}m \mathbf{v}^2$:

```math
T = \frac{1}{2}m \left[ \mathbf{v}' + \mathbf{v}_{rot} \right] 
= \frac{1}{2}m \left[ L \dot{\theta} \cos(\theta)\hat{x}' + L \dot{\theta} \sin(\theta)\hat{z}' + \Omega \left(\omega_1 +  L \sin(\theta) \right) \hat{y}'\right]^2 
= \frac{1}{2}m \left[ \left(L \dot{\theta} \cos(\theta)\right)^2 + \left(L \dot{\theta} \sin(\theta)\right)^2 + \left( \Omega \left(\omega_1 +  L \sin(\theta) \right) \right)^2 \right] 
= \frac{1}{2}m \left[ L^2 \dot{\theta}^2 + \Omega^2 \left( \omega_1 +  L \sin(\theta)\right)^2 \right]
```
...
# ╔═╡ c0f97376-0d39-49b4-ab7c-748b2dde9d1d
md"""### Equation of Motion
...
The expressions in the Lagrangian Equation of Motion for the rotating pendulum are as follows:

```math
\frac{\partial L}{\partial \theta} = - L ~ g ~ m ~ \sin\left( \theta\left( t \right) \right) + \Omega^{2} ~ L ~ \left( \omega + L ~ \sin\left( \theta\left( t \right) \right) \right) ~ m ~ \cos\left( \theta\left( t \right) \right)
```

```math
\frac{\partial L}{\partial \dot{\theta}} = L^{2} ~ m ~ \frac{\mathrm{d} ~ \theta\left( t \right)}{\mathrm{d}t}
```
"""

# ╔═╡ 12bb67d4-6fed-4e87-ba54-76385e26938f
md"""

Therefore, the expanded Lagrangian Equation of Motion is as follows: 

"""

# ╔═╡ 7ef00f31-f7cb-46cb-8287-f192e8ed6754
begin
...
# ╔═╡ 23388ccf-ab15-4e88-84fd-19c9ed0516ad
md"""### Differential Equation for Acceleration

The differential equation for the acceleration of the pendulum must be isolated from the EOM. The differential equation takes the form:

$\ddot{\theta} = f(\theta)$

Therefore, the acceleration is given by the following expression
"""
...
A numerical model can be used to solve for the acceleration based on the following input parameters: θ, D(θ), g, L, m, Ω, ω (though for this problem, m and L are constant)
...
# pass input parameters to solve for acceleration
...
manifest_format = "2.0"
project_hash = "0fc96c9c2f0c66e25a04e5f60fb7cac8fa35f687"
...
git-tree-sha1 = "1e487940a06ed9562de685e5f336633cdc14e011"
...
git-tree-sha1 = "df16470883cb5c2b2c2f3384176ffe5f83aced33"
